...
from loader import MoleculeDataset, graph_data_obj_to_nx_simple
...
def main():
    ...
    for dataset_name in downstream_dir:
        ...
        G_list = []
        for i in range(len(dataset.raw_file_names)):
            G_list.append(graph_data_obj_to_nx_simple(dataset[i]))
        model = GL2Vec()
        model.fit(G_list)
